...
export function TodoList() {
  ...
  const onSubmit = async (data: TodoInput) => {
    try {
      ...
      // Error is caught and set in useTodos state
    }
  }
      ...
      <Form {...form}>
        ...
        >
              ...
              <FormItem className="flex-1 space-y-0">
                <FormControl>
                  <Input
                    ...
                    className="h-12 flex-1 rounded-none border-0 bg-background px-4 text-sm tracking-[0.16px] placeholder:text-muted-foreground/60 focus-visible:ring-0 focus-visible:ring-offset-0"
                  />
                </FormControl>
          ...
          <Button
            ...
            className="ibm-transition h-12 rounded-none border-l border-border bg-primary px-6 text-sm tracking-[0.16px] text-primary-foreground hover:bg-[#0050e6]"
          >
      ...
      <div className="flex items-center border-b border-border">
        {(["all", "active", "done"] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`ibm-transition -mb-px border-b-2 px-5 py-3 text-sm tracking-[0.16px] capitalize ${
              filter === f
          ...
          >
            ...
            {f === "active" && activeCount > 0 && (
              <Badge
                variant="secondary"
                className="ml-2 rounded-none px-1.5 py-0 text-xs"
              >
                {activeCount}
              </Badge>
            )}
      ...
      <div className="border-x border-border">
        ...
        {error && (
          <div className="border-b border-destructive/20 bg-destructive/5 px-4 py-3">
            <p className="text-sm tracking-[0.16px] text-destructive">
              {error}
            </p>
          </div>
        )}
        ...
        {!loading && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="mt-2 text-sm tracking-[0.16px] text-muted-foreground">
              {filter === "all"
